...
interface ProductFormProps {
  ...
  onSubmit: (data: ProductFormData, imageIds: string[]) => Promise<void>;
  onCancel: () => void;
  submitLabel?: string;
  showIsSold?: boolean;
  isLoading?: boolean;
...
const ProductForm = ({
  ...
  useEffect(() => {
    if (initialImageIds.length > 0) {
      ...
      Promise.all(
        initialImageIds.map(async (id) => {
          try {
            const response = await imageApi.getById(id);
            return { id: response.id, url: response.image_url };
          } catch {
            return { id, url: imageApi.getImageUrl(id) };
          }
        })
      )
        .then(setImages)
  ...
  return (
    <form onSubmit={handleSubmit(handleFormSubmit)}>
      {/* Image Uploader */}
      {isLoadingImages ? (
        <div className="mb-6 h-24 flex items-center justify-center text-text-muted">
          {t.common.loading}
        </div>
      ) : (
